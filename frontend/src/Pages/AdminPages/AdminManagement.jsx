...
const AdminManagement = () => {
  ...
  useEffect(() => {
    const fetchAdmins = async () => {
      try {
        const response = await axios.get('http://35.232.49.147:3000/api/admin');
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`http://35.232.49.147:3000/api/admin/delete/${id}`);
  ...
  const handleUpdate = async (formData) => {
    ...
    try {
      const response = await axios.put(
        `http://35.232.49.147:3000/api/admin/update/${adminToUpdate._id}`,
        formData
      );
  ...
  const handleSubmit = async (e) => {
    ...
    if (isUpdating) {
      ...
      try {
        const response = await axios.post('http://35.232.49.147:3000/api/admin/add', formData);
